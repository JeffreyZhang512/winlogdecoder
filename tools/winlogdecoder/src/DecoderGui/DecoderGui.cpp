...
void DecoderGui::tableWidget_cellDoubleClicked(int row, int col)
{
    ...
    QString fileName = fileList[row].fileName();
    QString txtFileName = fileList[row].path() + QString("/output/") + fileName + QString(".xml.txt");
    ...
    if (index != -1)
    {
        ...
        case DECODER_STATE_SUCCESS:
        {
            ...
            if (pBar)
            {
                ...
                itemOut->setText(QString("output/") + QFileInfo(fileName).fileName() + QString(".xml.txt"));
